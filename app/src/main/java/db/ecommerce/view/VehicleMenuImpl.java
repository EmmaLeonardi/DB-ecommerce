...
public class VehicleMenuImpl {
    
    private final static String F = "/VehicleMenu.fxml";
    private final HBox root;

    public VehicleMenuImpl(Stage s) throws IOException {
        LoadFXML loader = new LoadFXMLImpl();
        var a = loader.load(F);
        if (a instanceof HBox) {
            this.root = (HBox) a;
        } else {
            var alert = new Alert(AlertType.ERROR, "Something went wrong loading the VehicleMenu");
            alert.show();

...
public class FactoryMenuImpl {

    private final static String F = "/FactoryMenu.fxml";
    private final HBox root;

    public FactoryMenuImpl(Stage s) throws IOException {
        LoadFXML loader = new LoadFXMLImpl();
        var a = loader.load(F);
        if (a instanceof HBox) {
            this.root = (HBox) a;
        } else {
            var alert = new Alert(AlertType.ERROR, "Something went wrong loading the FactoryMenu");
            alert.show();
